...
const WeeklyBreakdown = ({ onBack }) => {
        ...
        if (!dailySnapshot.empty) {
          ...
          Object.keys(data).forEach(key => {
            if (key.includes('Checklist') && Array.isArray(data[key])) {
              ...
              dayData.stats.totalTasks += checklist.length;
              dayData.stats.completedTasks += checklist.filter(item => item.completed).length;
              
              // Count by category
              checklist.forEach(item => {
                if (item.category) {
                  dayData.stats.categories[item.category] = dayData.stats.categories[item.category] || { total: 0, completed: 0 };
            ...
            } else if (key.includes('Time') && data[key]) {
              dayData.stats.timeSpent += parseInt(data[key]) || 0;
            }
          });
          
          dayData.stats.completionRate = dayData.stats.totalTasks > 0 
            ? (dayData.stats.completedTasks / dayData.stats.totalTasks * 100).toFixed(1)
            : 0;
        }
  ...
  const calculateWeekTotals = () => {
    ...
    Object.values(weeklyData).forEach(dayData => {
      totalTasks += dayData.stats.totalTasks;
      completedTasks += dayData.stats.completedTasks;
      totalTime += dayData.stats.timeSpent;
      
      Object.entries(dayData.stats.categories).forEach(([category, stats]) => {
        categoryTotals[category] = categoryTotals[category] || { total: 0, completed: 0 };
        categoryTotals[category].total += stats.total;
        categoryTotals[category].completed += stats.completed;
      });
    });
        ...
        <>
          ...
          <div className="week-summary">
            ...
            <div className="summary-grid">
              ...
              <div className="summary-card">
                <div className="summary-number">{weekTotals.completedTasks}</div>
                <div className="summary-label">Completed</div>
              </div>
              <div className="summary-card">
                <div className="summary-number">{weekTotals.completionRate}%</div>
          ...
          {Object.keys(weekTotals.categoryTotals).length > 0 && (
            <div className="category-breakdown">
              ...
              <div className="categories-grid">
                {Object.entries(weekTotals.categoryTotals).map(([category, stats]) => {
          ...
          <div className="daily-breakdown">
            ...
            <div className="days-grid">
              {weekDates.map((date, index) => {
                const dateStr = formatDateForFirebase(date);
                const dayData = weeklyData[dateStr] || { stats: { totalTasks: 0, completedTasks: 0, completionRate: 0, timeSpent: 0 } };
                  ...
                  <div key={dateStr} className={`day-card ${isToday ? 'today' : ''}`}>
                    ...
                    <div className="day-stats">
                      ...
                      <div className="stat-row">
                        ...
                        <span className="stat-value">{dayData.stats.completionRate}%</span>
                      </div>
                      <div className="stat-row">
                        <span className="stat-label">Time:</span>
                        <span className="stat-value">{dayData.stats.timeSpent}m</span>
                      </div>
                    </div>
                    ...
                    <div className="completion-bar">
                      <div 
                        ...
                        style={{ width: `${dayData.stats.completionRate}%` }}
                      ></div>
                    </div>

                    {/* Mini category breakdown */}
                    {Object.keys(dayData.stats.categories).length > 0 && (
                      <div className="day-categories">
                        {Object.entries(dayData.stats.categories).slice(0, 3).map(([category, stats]) => (
                          <div key={category} className={`mini-category ${category}`}>
                            <span className="mini-category-name">{category}</span>
                            <span className="mini-category-count">{stats.completed}/{stats.total}</span>
                          </div>
                        ))}
                      </div>
                    )}
          ...
          <div className="quick-actions">
            <button onClick={() => loadWeeklyData(selectedWeek)} className="refresh-btn">
              🔄 Refresh Data
            </button>
            <button onClick={() => setSelectedWeek(getThisWeekStart())} className="current-week-btn">
              📅 Current Week
            </button>
          </div>
        </>
